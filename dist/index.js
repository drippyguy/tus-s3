...
class S3Store extends utils_1.DataStore {
    ...
    async saveMetadata(upload, uploadId) {
        ...
        await this.client.putObject({
            ...
            Metadata: {
                'upload-id': uploadId,
                'tus-version': utils_1.TUS_RESUMABLE,
            },
        });
    ...
    async create(upload) {
        ...
        const request = {
            Bucket: this.bucket,
            Key: upload.id,
            Metadata: { 'tus-version': utils_1.TUS_RESUMABLE },
        };
    ...
    async write(src, id, offset) {
        ...
        if (incompletePart) {
            ...
            src = new multistream_1.default([incompletePart.createReader({ cleanUpOnEnd: true }), src]);
        }
