...
export class S3Store extends DataStore {
  ...
  private async getMetadata(id: string): Promise<MetadataValue> {
    ...
    const metadata: MetadataValue = {
      "upload-version": Metadata?.["upload-version"] as string,
  ...
  public async create(upload: Upload) {
    ...
    const request: AWS.CreateMultipartUploadCommandInput = {
      ...
      Metadata: { "upload-version": TUS_RESUMABLE },
    };
  ...
  async deleteExpired(): Promise<number> {
    ...
    while (isTruncated) {
      ...
      const objectsToDelete = expiredUploads.reduce((all, expiredUpload) => {
        all.push(
          {
            key: this.getFilePath(this.infoKey(expiredUpload.Key as string)),
          },
          {
            key: this.getFilePath(this.partKey(expiredUpload.Key as string, true)),
          }
        );
        return all;
      }, [] as { key: string }[]);
